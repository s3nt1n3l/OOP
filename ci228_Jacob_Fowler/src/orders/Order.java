 ...
 * progressed through the system. These stages are: <BR>
...
public class Order implements OrderProcessing {
	...
	/**
	 * Generates a unique order number would be good to recycle numbers after
	 * 999
	 * 
	 ...
	 * 
	 * @param bought
	 *            A new order that is to be processed
	 */
	...
	public synchronized Basket getOrderToPick() throws OrderException {
		...
		for (Folder folder : folders) {
			if (folder.getState() == State.Waiting) {
				folder.newState(State.BeingPicked);
	...
	/**
	 * Informs the order processing system that the order has been picked and
	 * the products are now being delivered to the collection desk
	 * 
	 * @param orderNum
	 *            The order that has been picked
	...
	public synchronized boolean informOrderPicked(int orderNum) throws OrderException {
		...
		for (Folder folder : folders) {
			if (folder.getState() == State.BeingPicked && folder.getBasket().getOrderNum() == orderNum) {
	...
	/**
	 * Informs the order processing system that the order has been collected by
	 * the customer
	 * 
	...
	public synchronized boolean informOrderCollected(int orderNum) throws OrderException {
		...
		for (Folder folder : folders) {
			if (folder.getState() == State.ToBeCollected && folder.getBasket().getOrderNum() == orderNum) {
				writeFile(folder);
	...
	public void writeFile(Folder folder) { //Opens Logs.txt, adds log then closes
		try {
			File log = new File("Logs.txt");

			FileWriter writeLog = new FileWriter(log, true);
			writeLog.append(folder.getBasket().getOrderNum() + "\n");
			writeLog.append(folder.getBasket().getDetails() + "\n");
			writeLog.close();
	...
	/**
	 * Returns information about all the orders (there order number) in the
	 * order processing system This consists of a map with the following keys:
	 * 
	 * <PRE>
	 ...
	 * 
	 * @param inState
	 *            The state to find order numbers in
	...
	private List<Integer> orderNums(State inState) {
		return folders.stream().filter(folder -> folder.getState() == inState)
				.map(folder -> folder.getBasket().getOrderNum()).collect(Collectors.toList());
	}
